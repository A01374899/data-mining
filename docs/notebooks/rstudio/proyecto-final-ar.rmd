...
Escribe la fecha actual (**NOTE-01**): 03 de junio 20201
...
```{r}
NOMBRE_COMPLETO = "Eduardo Jaramillo Diosdado"
DIGITOS_MATRICULA = 01374899
```
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): Que no existe relación entre el producto de pañales con otro artículo 
- Calcula el valor de la métrica support para la regla `{beer} => {nuts}` (**NOTE-20**): 1/5
- Calcula el valor de la métrica confidence para la regla `{beer} => {nuts}` (**NOTE-21**): 1/3
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Porque las métricas de support y condfidence excluyen a la regla 
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Representa cada una de las sesiones del dataset y su propósito es distinguir el número de sesiones que los usuarios tiene en spotify para poder hacer recomendaciones 
- ¿Cuál es el propósito de `track id`? (**NOTE-24**): Representar cada una de las canciones, distinguir cuáles son las canciones que escuchan varios usuarios y hacer recomendaciones con base en sus preferencias 
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Sí, ya que representan el comportamiento que el usuario tiene en spotify cuando escucha música. Se pueden hacer asociaciones si se entiende la relación de las canciones con los gustos por medio de las canciones saltadas, pausas por canción, etc.
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): t_bacf06d3-9185-4183-84ea-ff0db51475ce - 879 
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): Itemset 20 con 1558
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Los usuarios tienen en promedio 14.58 sesiones 
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): Ninguna de las reglas de asociación cumple con los parámetros que se establecieron bajo los valores de confidence y support. 
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Que se pierda presición en el análisis ya que se tendrá un mayor número de reglas de asociación que pueden no ser tan certeras. 
...
- Provee el enlace al video aquí: **NOTE-30** Enlace: https://drive.google.com/drive/folders/11JXFXj-wP_0Lfwd6qg9XDMAJ9ZcZwDs5?usp=sharing
